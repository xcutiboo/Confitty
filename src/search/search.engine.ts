import type { SearchableItem, SearchMatch, SearchResult } from "./search.types";
...
function scoreItem(item: SearchableItem, query: string): number {
	...
	if (label.split(/\s+/).some((word) => word.startsWith(q)))
		return Score.LABEL_WORD_PREFIX;
	...
	if (description.split(/\s+/).some((word) => word.toLowerCase().startsWith(q)))
...
export function highlight(text: string, query: string): SearchMatch[] {
	...
	if (idx + q.length < text.length)
		segments.push({ text: text.slice(idx + q.length), matched: false });
