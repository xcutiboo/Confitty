...
export class PresetSelectorComponent {
  ...
  applyPreset(preset: ConfigPreset): void {
    ...
    // Reset all config sections to defaults before applying preset to prevent stale values
    mergedConfig.tab_bar = structuredClone(DEFAULT_KITTY_CONFIG.tab_bar);
    mergedConfig.cursor = structuredClone(DEFAULT_KITTY_CONFIG.cursor);
    mergedConfig.fonts = structuredClone(DEFAULT_KITTY_CONFIG.fonts);
    mergedConfig.scrollback = structuredClone(DEFAULT_KITTY_CONFIG.scrollback);
    mergedConfig.mouse = structuredClone(DEFAULT_KITTY_CONFIG.mouse);
    mergedConfig.performance = structuredClone(DEFAULT_KITTY_CONFIG.performance);
    mergedConfig.bell = structuredClone(DEFAULT_KITTY_CONFIG.bell);
    mergedConfig.window_layout = structuredClone(DEFAULT_KITTY_CONFIG.window_layout);
    mergedConfig.advanced = structuredClone(DEFAULT_KITTY_CONFIG.advanced);
