...
const KEY_TO_SECTION: Readonly<Record<string, keyof KittyConfigAST>> = {
  cursor_shape: 'cursor', cursor_blink_interval: 'cursor', cursor_stop_blinking_after: 'cursor',
  cursor_beam_thickness: 'cursor', cursor_underline_thickness: 'cursor',

  font_size: 'fonts', font_family: 'fonts', bold_font: 'fonts', italic_font: 'fonts',
  bold_italic_font: 'fonts', disable_ligatures: 'fonts', force_ltr: 'fonts', box_drawing_scale: 'fonts',

  scrollback_lines: 'scrollback', scrollback_pager: 'scrollback',
  scrollback_pager_history_size: 'scrollback', scrollback_fill_enlarged_window: 'scrollback',
  wheel_scroll_multiplier: 'scrollback', wheel_scroll_min_lines: 'scrollback',
  touch_scroll_multiplier: 'scrollback',

  mouse_hide_wait: 'mouse', url_style: 'mouse', open_url_with: 'mouse', url_prefixes: 'mouse',
  detect_urls: 'mouse', copy_on_select: 'mouse', strip_trailing_spaces: 'mouse',
  show_hyperlink_targets: 'mouse', underline_hyperlinks: 'mouse', focus_follows_mouse: 'mouse',

  repaint_delay: 'performance', input_delay: 'performance', sync_to_monitor: 'performance',

  enable_audio_bell: 'bell', visual_bell_duration: 'bell', visual_bell_color: 'bell',
  window_alert_on_bell: 'bell', bell_on_tab: 'bell', command_on_bell: 'bell',

  remember_window_size: 'window_layout', initial_window_width: 'window_layout',
  initial_window_height: 'window_layout', window_padding_width: 'window_layout',
  window_margin_width: 'window_layout', single_window_margin_width: 'window_layout',
  window_border_width: 'window_layout', hide_window_decorations: 'window_layout',
  confirm_os_window_close: 'window_layout', draw_minimal_borders: 'window_layout',
  inactive_text_alpha: 'window_layout', window_resize_step_cells: 'window_layout',
  window_resize_step_lines: 'window_layout', active_border_color: 'window_layout',
  inactive_border_color: 'window_layout', bell_border_color: 'window_layout',

  tab_bar_style: 'tab_bar', tab_bar_edge: 'tab_bar', tab_bar_min_tabs: 'tab_bar',
  tab_title_template: 'tab_bar', tab_powerline_style: 'tab_bar', tab_separator: 'tab_bar',
  tab_bar_background: 'tab_bar', active_tab_foreground: 'tab_bar',
  active_tab_background: 'tab_bar', inactive_tab_foreground: 'tab_bar',
  inactive_tab_background: 'tab_bar',

  shell: 'advanced', editor: 'advanced', close_on_child_death: 'advanced',
  allow_remote_control: 'advanced', update_check_interval: 'advanced',
...
const COLOR_OVERRIDES = new Set([
  ...
  'background_opacity', 'background_blur', 'background_image', 'dim_opacity',
]);
...
export class PresetSelectorComponent {
  ...
  readonly paletteKeys: readonly string[] = [
    ...
    'color5', 'color6', 'color11', 'color14',
  ];

  applyPreset(preset: ConfigPreset): void {
    const next = structuredClone(this.configStore.configState()) as unknown as Record<string, unknown>;
    ...
    if (preset.category === 'theme') {
      ...
      const defaultTabBar = DEFAULT_KITTY_CONFIG.tab_bar as unknown as Record<string, unknown>;
      for (const key of TAB_BAR_COLOR_KEYS) {
        tabBarSlice[key] = defaultTabBar[key];
      }

      (next['window_layout'] as Record<string, unknown>)['active_border_color'] =
    ...
    const presetSetsTabColors = Object.keys(preset.config).some(k => TAB_BAR_COLOR_KEYS.has(k));
