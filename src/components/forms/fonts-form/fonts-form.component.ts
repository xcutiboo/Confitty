  ...
  template: `
    ...
    >
      <div class="form-group">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
            @for (font of fontPresetsService.getPopularFonts(); track font.family) {
              <button
                ...
                class="px-3 py-2.5 rounded-lg text-xs hover:bg-kitty-primary hover:text-kitty-dark transition-colors border border-kitty-border leading-relaxed"
        ...
        <details class="mb-3">
          <summary
            class="text-xs font-medium text-kitty-accent cursor-pointer hover:text-kitty-secondary"
          >
          ...
          <div class="grid grid-cols-2 md:grid-cols-5 gap-2 mt-3">
            @for (font of fontPresetsService.getAllFonts(); track font.family) {
              <button
                ...
                [class.bg-kitty-surface-light]="fonts().font_family !== font.family"
                [class.text-kitty-text]="fonts().font_family !== font.family"
                class="px-2.5 py-2 rounded text-xs hover:bg-kitty-primary hover:text-kitty-dark transition-colors border border-kitty-border leading-relaxed"
                [style.font-family]="getFontFamily(font)"
                [title]="font.description"
              >
                {{ font.name }}
      ...
      <div class="form-group">
        <label class="block text-sm font-medium text-kitty-text mb-2">
          ...
          <span class="text-kitty-text-dim text-xs ml-2"
            >Multi-character programming ligatures (==, ->, =>, etc.)</span
          >
        </label>
        <select
          ...
          <label class="flex items-center gap-3 cursor-pointer">
            ...
            <div>
              ...
              <p class="text-kitty-text-dim text-xs mt-0.5">
                Enable when using external BIDI programs like GNU FriBidi.
              </p>
            </div>
          </label>
        </div>
        ...
        <div class="form-group" [class.opacity-60]="!textCompositionStrategyAvailable()">
          <div class="flex items-center gap-2 mb-2">
            <label class="block text-sm font-medium text-kitty-text">
              ...
              <span class="text-kitty-text-dim text-xs ml-2"
                >How glyphs are composited onto the background</span
              >
            </label>
      ...
      @if (helper.advancedMode()) {
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="form-group" [class.opacity-60]="!underlineExclusionAvailable()">
              <div class="flex items-center gap-2 mb-2">
                <label class="block text-sm font-medium text-kitty-text">
                  Underline Exclusion
                  <span class="text-kitty-text-dim text-xs ml-2"
                    >Gap padding when underlines intersect descenders (0 = no gaps)</span
                  >
                </label>
            ...
            <div class="form-group">
              <label class="block text-sm font-medium text-kitty-text mb-2">
                ...
                <span class="text-kitty-text-dim text-xs ml-2"
                  >Minimum contrast ratio (0 = disabled)</span
                >
              </label>
              <app-number-input
        ...
        >
          <div class="flex items-center gap-2 mb-1">
            <h3 class="text-lg font-semibold text-kitty-text">
              OpenType Features & Symbol Mapping
            </h3>
          ...
          <p class="text-kitty-text-dim text-sm mb-4">
            Advanced per-face OpenType feature flags and custom Unicode range-to-font mappings.
            Requires Kitty 0.19+
          </p>
          ...
          <div class="space-y-4">
            <div class="form-group">
              <label class="block text-sm font-medium text-kitty-text mb-2">
                ...
                <span class="text-kitty-text-dim text-xs ml-2"
                  >One per line - e.g.
                  <code class="bg-kitty-bg px-1 rounded font-mono"
                    >FiraCode-Regular +zero</code
                  ></span
                >
              </label>
              <textarea
                [ngModel]="
                  fonts().font_features.join(
                    '
'
                  )
                "
            ...
            <div class="form-group">
              <label class="block text-sm font-medium text-kitty-text mb-2">
                Symbol Map
                <span class="text-kitty-text-dim text-xs ml-2"
                  >Map Unicode ranges to specific fonts (for Powerline/Nerd Fonts)</span
                >
              </label>
              <textarea
                [ngModel]="
                  fonts().symbol_map.join(
                    '
'
                  )
                "
              ...
              <p class="text-xs text-kitty-text-dim mt-2">
                <strong class="text-kitty-text">Powerline symbols:</strong>
                U+E0A0-U+E0A3,U+E0C0-U+E0C7 •
                <strong class="text-kitty-text">Nerd Fonts:</strong> Use
                <code class="bg-kitty-bg px-1 rounded">Symbols Nerd Font</code> or
                <code class="bg-kitty-bg px-1 rounded">Symbols Nerd Font Mono</code>
              </p>
            </div>
            ...
            <div class="form-group">
              <label class="block text-sm font-medium text-kitty-text mb-2">
                Narrow Symbols
                <span class="text-kitty-text-dim text-xs ml-2"
                  >Unicode ranges to render at narrower widths (e.g. emoji)</span
                >
              </label>
              <textarea
                [ngModel]="
                  fonts().narrow_symbols.join(
                    '
'
                  )
                "
            ...
            <div class="form-group" [class.opacity-60]="!fontFeaturesAvailable()">
              <label class="block text-sm font-medium text-kitty-text mb-2">
                Modify Font
                <span class="text-kitty-text-dim text-xs ml-2"
                  >Fine-tune font metrics (one per line: cell_width 110%, baseline +2px)</span
                >
              </label>
              <textarea
                [ngModel]="
                  fonts().modify_font.join(
                    '
'
                  )
                "
              ...
              ></textarea>
              <p class="text-xs text-kitty-text-dim mt-2">
                <strong class="text-kitty-text">Valid metrics:</strong> cell_width, cell_height,
                baseline, underline_position, underline_thickness, strikethrough_position,
