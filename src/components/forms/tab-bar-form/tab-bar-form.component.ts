...
@Component({
    ...
    <app-form-section
      title="Tab Bar"
      description="Customize the appearance and behavior of the tab bar"
      icon="copy"
    >
          ...
          <div class="form-group">
            <label class="block text-sm font-medium text-kitty-text mb-2">
              ...
              <span class="text-kitty-text-dim text-xs ml-2"
                >Which tab to focus when current tab closes</span
              >
            </label>
            <select
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="form-group">
            <label class="block text-sm font-medium text-kitty-text mb-2">
              Powerline Style
              <span class="text-kitty-text-dim text-xs ml-2"
                >Arrow shape for 'powerline' style</span
              >
            </label>
            <select
        ...
        @if (tabBar().tab_bar_style === 'powerline') {
          <div class="bg-kitty-bg/60 border border-kitty-primary/20 rounded-lg p-4 space-y-3">
            <div class="flex items-start gap-2">
              <svg
                class="w-5 h-5 text-kitty-primary flex-shrink-0 mt-0.5"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <div class="text-sm text-kitty-text leading-relaxed">
                <strong class="text-kitty-primary">Powerline Setup:</strong> For best results, use
                <code class="bg-kitty-bg px-1.5 py-0.5 rounded font-mono text-xs">symbol_map</code>
                in the Fonts section to map Powerline symbols (U+E0A0-U+E0A3, U+E0C0-U+E0C7) to a
                Nerd Font. This avoids needing patched fonts.
              </div>
            </div>
            <div class="text-xs text-kitty-text-dim space-y-1.5">
              <div>
                <strong class="text-kitty-text">Example symbol_map:</strong>
                <code class="bg-kitty-surface px-1.5 py-0.5 rounded font-mono"
                  >U+E0A0-U+E0A3,U+E0C0-U+E0C7 Symbols Nerd Font</code
                >
              </div>
              <div>
                <strong class="text-kitty-text">Color formatting:</strong> Use
                <code class="bg-kitty-surface px-1.5 py-0.5 rounded font-mono"
                  >&#123;fmt.fg._fff&#125;</code
                >
                in title templates for custom colors
              </div>
              <div>
                <strong class="text-kitty-text">Works great with:</strong> powerlevel10k, starship,
                zsh themes
              </div>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="form-group">
            <label class="block text-sm font-medium text-kitty-text mb-2">
              Tab Title Template
              <span class="text-kitty-text-dim text-xs ml-2"
                >Variables: &#123;title&#125;, &#123;index&#125;, &#123;num_windows&#125;</span
              >
            </label>
            <input
              type="text"
          ...
          <div class="form-group">
            <label class="block text-sm font-medium text-kitty-text mb-2">
              ...
              <span class="text-kitty-text-dim text-xs ml-2"
                >Override for active tab. 'none' uses tab_title_template</span
              >
            </label>
            <input
              type="text"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="form-group">
            <label class="block text-sm font-medium text-kitty-text mb-2">
              Activity Symbol
              <span class="text-kitty-text-dim text-xs ml-2"
                >Shown in tab title when activity occurs. 'none' to disable.</span
              >
            </label>
            <input
              type="text"
          ...
          <div class="space-y-4">
            ...
            @if (helper.advancedMode()) {
              <div class="form-group">
                ...
                >
                <select
                  [(ngModel)]="tabBar().active_tab_font_style"
                  (ngModelChange)="helper.updateField('active_tab_font_style', $event)"
          ...
          <div class="space-y-4">
            ...
            @if (helper.advancedMode()) {
              <div class="form-group">
                ...
                >
                <select
                  [(ngModel)]="tabBar().inactive_tab_font_style"
                  (ngModelChange)="helper.updateField('inactive_tab_font_style', $event)"
                  class="w-full px-3 py-1.5 text-sm bg-kitty-bg border border-kitty-border rounded-lg text-kitty-text focus:outline-none focus:ring-2 focus:ring-kitty-primary"
                >
                  <option value="normal">Normal</option>
                  <option value="bold">Bold</option>
