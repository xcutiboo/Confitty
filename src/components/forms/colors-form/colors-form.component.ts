      ...
      <div class="form-group">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          @for (theme of colorThemesService.getAllThemes(); track theme.name) {
            ...
            >
              <div class="font-medium text-kitty-text group-hover:text-kitty-primary text-sm">
                {{ theme.name }}
              </div>
              ...
              <div class="flex gap-1 mt-2">
                @for (i of [0, 1, 2, 3, 4, 5, 6, 7]; track i) {
                  <div
                    ...
                    [style.background-color]="getThemeColor(theme.colors, i)"
                  ></div>
                }
              </div>
            </button>
      ...
      <div class="border-t border-kitty-border pt-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="form-group">
            ...
            <app-color-input
              ...
              (valueChange)="helper.updateField('foreground', $event)"
            />
          </div>
          ...
          <div class="form-group">
            ...
            <app-color-input
              ...
              (valueChange)="helper.updateField('background', $event)"
            />
          </div>
        </div>
        ...
        <div class="mt-6 form-group">
          <label class="block text-sm font-medium text-kitty-text mb-2">
            ...
            <span class="text-kitty-accent font-bold ml-2"
              >{{ (colors().background_opacity * 100).toFixed(0) }}%</span
            >
            <span class="text-kitty-text-dim text-xs ml-2"
              >Requires compositor support (e.g. picom, KWin)</span
            >
          </label>
          <app-slider-input
        ...
        @if (helper.advancedMode()) {
              ...
              <label class="flex items-center gap-3 cursor-pointer">
                ...
                <div>
                  <span class="text-sm font-medium text-kitty-text"
                    >Dynamic Background Opacity</span
                  >
                  <p class="text-kitty-text-dim text-xs mt-0.5">
                    Allow changing opacity at runtime with Ctrl+Shift+A → L/M
                  </p>
                </div>
              </label>
          ...
          <div class="border-t border-kitty-border pt-6 mt-6">
            ...
            @if (
              colors().background_image !== 'none' &&
              colors().background_image &&
              colors().background_opacity < 1
            ) {
              <div class="mb-4 p-3 bg-kitty-accent/10 border border-kitty-accent/30 rounded-lg">
                <p class="text-sm text-kitty-accent">
                  ...
                  Kitty 0.43.0+: background_opacity no longer applies to background_image. Bake
                  alpha into the image file for transparency.
                </p>
              </div>
            }
            <div class="space-y-4">
              <div class="form-group">
                <label class="block text-sm font-medium text-kitty-text mb-2">
                  ...
                  <span class="text-kitty-text-dim text-xs ml-2"
                    >PNG image to display behind terminal content</span
                  >
                </label>
                <input
                  type="text"
                ...
                <div class="form-group">
                  <label class="flex items-center gap-3 cursor-pointer mt-6">
                    ...
                    <div>
                      ...
                      <p class="text-kitty-text-dim text-xs mt-0.5">
                        Use linear interpolation when scaling
                      </p>
                    </div>
                  </label>
                </div>
          ...
          <div class="border-t border-kitty-border pt-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div class="form-group">
                <label class="block text-sm font-medium text-kitty-text mb-2">
                  Background Tint
                  <span class="text-kitty-accent ml-2"
                    >{{ (colors().background_tint * 100).toFixed(0) }}%</span
                  >
                </label>
                <app-slider-input
              ...
              <div class="form-group">
                <label class="block text-sm font-medium text-kitty-text mb-2">
                  ...
                  <span class="text-kitty-accent ml-2"
                    >{{ (colors().background_tint_gaps * 100).toFixed(0) }}%</span
                  >
              ...
              <div class="form-group">
                <label class="block text-sm font-medium text-kitty-text mb-2">
                  Dim Opacity
                  <span class="text-kitty-accent ml-2"
                    >{{ (colors().dim_opacity * 100).toFixed(0) }}%</span
                  >
          ...
          <div class="border-t border-kitty-border pt-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="form-group">
                <label class="block text-sm font-medium text-kitty-text mb-2"
                  >Selection Foreground</label
                >
                <app-color-input
                  [value]="colors().selection_foreground || '#000000'"
                  (valueChange)="helper.updateField('selection_foreground', $event)"
                />
              </div>
              ...
              <div class="form-group">
                ...
                >
                <app-color-input
                  [value]="colors().selection_background || '#ffffff'"
                  (valueChange)="helper.updateField('selection_background', $event)"
                />
              </div>
            </div>
          </div>
          ...
          <div
            class="border-t border-kitty-border pt-6"
            [class.opacity-60]="!transparentBackgroundColorsAvailable()"
          >
            ...
            <p class="text-kitty-text-dim text-sm mb-4">
              Set per-color transparency. Format: color@opacity (e.g., red@0.5, #ff0000@0.3)
            </p>
            <div class="form-group">
              <textarea
                [ngModel]="
                  colors().transparent_background_colors.join(
                    '
'
                  )
                "
          ...
          <div class="border-t border-kitty-border pt-6">
            ...
            <p class="text-kitty-text-dim text-sm mb-4">
              Colors 16-255 for terminal applications that support the full 256-color palette.
            </p>
            <details class="group">
              <summary
                class="flex items-center gap-2 cursor-pointer text-sm font-medium text-kitty-accent hover:text-kitty-secondary transition-colors"
              >
          ...
          <div class="border-t border-kitty-border pt-6">
            ...
            <p class="text-kitty-text-dim text-sm mb-4">
              Colors used when highlighting text with the
              <code class="bg-kitty-bg px-1 rounded font-mono text-xs">create_marker</code> action.
            </p>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
