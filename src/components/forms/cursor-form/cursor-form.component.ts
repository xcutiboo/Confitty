    ...
    <app-form-section title="Cursor" description="Customize cursor shape, color, and animation">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="form-group">
            <label class="block text-sm font-medium text-kitty-text mb-2">
              Cursor Color
              <span class="text-kitty-text-dim text-xs ml-2">Hex value or 'none' for reverse-video effect</span>
            </label>
            <app-color-input 
          ...
          @if (helper.advancedMode()) {
            <div class="form-group">
              <label class="block text-sm font-medium text-kitty-text mb-2">
                Cursor Text Color
                <span class="text-kitty-text-dim text-xs ml-2">'background' inherits the cell's background color</span>
              </label>
              <app-color-input 
            ...
            <div class="form-group" [class.opacity-60]="!cursorShapeUnfocusedAvailable()">
              <div class="flex items-center gap-2 mb-2">
                <label class="block text-sm font-medium text-kitty-text">
                  Shape (Unfocused)
                  <span class="text-kitty-text-dim text-xs ml-2">Cursor appearance when window loses focus</span>
                </label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="form-group">
            <label class="block text-sm font-medium text-kitty-text mb-2">
              Blink Interval
              <span class="text-kitty-text-dim text-xs ml-2">Seconds between blinks (0 = solid, -1 = system default)</span>
            </label>
            <app-number-input
          ...
          @if (helper.advancedMode()) {
            <div class="form-group" [class.opacity-60]="!cursorStopBlinkingAvailable()">
              <div class="flex items-center gap-2 mb-2">
                <label class="block text-sm font-medium text-kitty-text">
                  Stop Blinking After
                  <span class="text-kitty-text-dim text-xs ml-2">Seconds of inactivity before cursor stops blinking (0 = never)</span>
                </label>
        ...
        @if (helper.advancedMode()) {
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="form-group">
              <label class="block text-sm font-medium text-kitty-text mb-2">
                Beam Thickness
                <span class="text-kitty-accent font-bold ml-2">{{ cursor().cursor_beam_thickness }}pt</span>
              </label>
              <app-slider-input 
            ...
            <div class="form-group">
              <label class="block text-sm font-medium text-kitty-text mb-2">
                Underline Thickness
                <span class="text-kitty-accent font-bold ml-2">{{ cursor().cursor_underline_thickness }}pt</span>
              </label>
              <app-slider-input 
          ...
          <div class="bg-kitty-bg/60 border border-kitty-primary/20 rounded-lg p-4 mt-6" [class.opacity-60]="!cursorTrailAvailable()">
            ...
            <p class="text-xs text-kitty-text-dim mb-4">Simulates smooth cursor movement by leaving phantom cursors during rapid jumps (requires GPU support)</p>
...
export class CursorFormComponent {
  ...
  readonly cursorTrailAvailable = computed(() => this.versionService.isOptionAvailable('cursor_trail'));
  readonly cursorStopBlinkingAvailable = computed(() => this.versionService.isOptionAvailable('cursor_stop_blinking_after'));
  readonly cursorShapeUnfocusedAvailable = computed(() => this.versionService.isOptionAvailable('cursor_shape_unfocused'));
  ...
  cursorTextColorForPicker(): string {
    return this.cursor().cursor_text_color.startsWith('#') ? this.cursor().cursor_text_color : '#111111';
  }
  ...
  setTrailDecay(value: string): void {
    const decay = value.split(/\s+/).map(Number).filter(n => !Number.isNaN(n));
    this.helper.updateField('cursor_trail_decay', decay.length > 0 ? decay : [0.1, 0.4]);
  }

  getTrailColor(): string {
    return this.cursor().cursor_trail_color?.startsWith('#') ? this.cursor().cursor_trail_color : '#cccccc';
  }
