...
const fg = (text: string, opts: Partial<Span> = {}): Span => ({ text, color: 'fg', ...opts });
const ansi = (text: string, color: AnsiKey, opts: Partial<Span> = {}): Span => ({ text, color, ...opts });
const dim = (text: string, opts: Partial<Span> = {}): Span => ({ text, color: 'fg', dim: true, ...opts });
...
export const SAMPLE_SESSION: Line[] = [
  { spans: [...prompt('~/dotfiles'), ansi('ls', 'color2', { bold: true }), fg(' -la')] },
  { spans: [dim('total 48')] },
  { spans: [
  ...
  { spans: [...prompt('~/dotfiles'), ansi('git', 'color2', { bold: true }), fg(' status -sb')] },
  { spans: [
    ansi('## ', 'color8'),
