...
export class TerminalWindowComponent {
	...
	private readonly windowCfg = computed(
		() => this.store.configState().window_layout,
	);
	...
	readonly showTabBar = computed(() => {
		...
		return (
			tb.tab_bar_style !== "hidden" && TAB_COUNT >= (tb.tab_bar_min_tabs ?? 2)
		);
	});
	...
	readonly windowStyles = computed(() => {
		...
		const pt = Math.max(
			FONT_SIZE_PT.min,
			Math.min(FONT_SIZE_PT.max, fonts.font_size),
		);
		const px = ptToPx(pt);
		...
		const borderWidth = showBorder
			? this.parseBorderWidth(w.window_border_width)
			: 0;
		const ligatures =
			fonts.disable_ligatures === "always" ? "none" : "contextual";
	...
	private shouldShowBorder(w: ReturnType<typeof this.windowCfg>): boolean {
		const explicit = (
			w as unknown as { draw_window_borders_for_single_window?: boolean }
	...
	private parseBorderWidth(value: string | undefined): number {
		...
		if (
			!trimmed ||
			trimmed === "0" ||
			trimmed.startsWith("0pt") ||
			trimmed === "0px"
		)
			return 0;
		if (trimmed.endsWith("pt"))
			return Math.max(1, Math.round(ptToPx(Number.parseFloat(trimmed))));
