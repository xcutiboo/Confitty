...
export class KittyVersionService {
	...
	readonly featureRequirements: Record<string, FeatureRequirement> = {
		...
		scrollbar_gap: {
			minVersion: "0.43.0",
			description: "Gap between scrollbar and screen",
			warning: "In cell widths",
		},
		scrollbar_handle_color: {
			minVersion: "0.43.0",
			description: "Color of the scrollbar handle",
		},
		scrollbar_handle_opacity: {
			minVersion: "0.43.0",
			description: "Opacity of scrollbar handle",
		},
		scrollbar_hitbox_expansion: {
			minVersion: "0.43.0",
			description: "Extra hit zone around handle",
		},
		scrollbar_hover_width: {
			minVersion: "0.43.0",
			description: "Handle width while hovering",
		},
		scrollbar_interactive: {
			minVersion: "0.43.0",
			description: "Allow click-and-drag scrolling",
		},
		scrollbar_jump_on_click: {
			minVersion: "0.43.0",
			description: "Jump on track click",
		},
		scrollbar_min_handle_height: {
			minVersion: "0.43.0",
			description: "Minimum handle height",
		},
		scrollbar_radius: {
			minVersion: "0.43.0",
			description: "Corner rounding for scrollbar",
		},
		...
		scrollbar_track_hover_opacity: {
			minVersion: "0.43.0",
			description: "Track opacity while hovering",
		},
		scrollbar_track_opacity: {
			minVersion: "0.43.0",
			description: "Default track opacity",
		},
		scrollbar_width: {
			minVersion: "0.43.0",
		...
		palette_generate: {
			minVersion: "0.47.0",
			description:
				"Generate the extended 256-color palette from the first 16 colors",
	...
	readonly deprecatedOptions: Record<
		string,
		{ replacement: string; since: string; autoConvert?: boolean }
	> = {
	...
	readonly breakingChanges: Record<string, string[]> = {
		"0.21.0": [
			"terminal_select_modifiers, rectangle_select_modifiers, open_url_modifiers removed. Use mouse_map instead.",
		],
		"0.22.0": ["background_opacity no longer affects background_image"],
		"0.26.0": [
			"adjust_baseline, adjust_line_height, adjust_column_width deprecated. Use modify_font instead.",
		],
		"0.36.3": [
			"second_transparent_bg removed. Use transparent_background_colors instead.",
		],
	...
	getBreakingChangesForVersion(version: string): string[] {
		...
		for (const [changeVersion, changeList] of Object.entries(
	...
	isFeatureAvailable(feature: string): boolean {
		...
		return (
			this.compareVersions(this.currentVersion(), requirement.minVersion) >= 0
		);
	}
	...
	getVersionRequirement(
		feature: string,
	): { minVersion: string; description: string; warning?: string } | null {
