...
export class ColorThemesService {
	...
	readonly themes: ColorTheme[] = [
		...
		theme(
			"Everforest Dark",
			"Comfortable warm-green dark theme for long coding sessions",
			[
				"#d3c6aa",
				"#2d353b",
				"#2d353b",
		...
		theme(
			"Kanagawa Wave",
			"Dark theme inspired by the paintings of Katsushika Hokusai",
			[
				"#dcd7ba",
				"#1f1f28",
				"#1f1f28",
		...
		theme(
			"Cyberdream",
			"High-contrast neon dark theme for terminal maximalists",
			[
				"#ffffff",
				"#16181a",
				"#16181a",
	...
	getTheme(name: string): ColorTheme | undefined {
		return (
			...
			this._customThemes().find((t) => t.name === name)
		);
	}
	...
	getThemesByCategory(): Record<string, ColorTheme[]> {
		...
		for (const theme of this.themes) {
			...
			if (name.includes("catppuccin")) {
				...
				name.includes("fox") ||
				name.includes("carbonfox") ||
				name.includes("terafox")
			) {
				categories["Nightfox"]!.push(theme);
			} else if (
				["vs code", "intellij", "github", "monokai classic"].some((n) =>
		...
		return Object.fromEntries(
			Object.entries(categories).filter(([, themes]) => themes.length > 0),
		);
	}
}
