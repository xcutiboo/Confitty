...
export class KittyParserService {
	...
	private parseEnvironmentVariable(
		config: KittyConfigAST,
		value: string,
	): void {
	...
	private isMouseKey(key: string): boolean {
		return (
			...
			].includes(key) || key.startsWith("pointer_shape")
		);
	}
	...
	private isOSSpecificKey(key: string): boolean {
		return (
			key.startsWith("macos_") ||
			key.startsWith("wayland_") ||
			key.startsWith("linux_")
		);
	}

	private parseFontConfig(
		config: KittyConfigAST,
		key: string,
		value: string,
	): void {
		switch (key) {
			...
			case "text_composition_strategy":
				config.fonts.text_composition_strategy =
					value as KittyTextCompositionStrategy;
				break;
	...
	private parseCursorConfig(
		...
		switch (key) {
			...
			case "cursor_shape_unfocused":
				config.cursor.cursor_shape_unfocused =
					value as KittyCursorShapeUnfocused;
				break;
	...
	private parseScrollbackConfig(
		...
		switch (key) {
			...
			case "scrollback_pager_history_size":
				config.scrollback.scrollback_pager_history_size = Number.parseInt(
					value,
					10,
				);
				break;
			case "scrollback_fill_enlarged_window":
				config.scrollback.scrollback_fill_enlarged_window =
			...
			case "scrollbar_interactive":
				config.scrollback.scrollbar_interactive =
					value === "yes" || value === "true";
				break;
			case "scrollbar_jump_on_click":
				config.scrollback.scrollbar_jump_on_click =
					value === "yes" || value === "true";
				break;
			case "scrollbar_min_handle_height":
				config.scrollback.scrollbar_min_handle_height =
			...
			case "scrollbar_track_hover_opacity":
				config.scrollback.scrollbar_track_hover_opacity =
					Number.parseFloat(value);
				break;
	...
	private parseMouseConfig(
		...
		switch (key) {
			...
			case "copy_on_select":
				...
				break;
			case "paste_actions":
				config.mouse.paste_actions = value
					.split(",")
					.map((v) => v.trim())
					.filter(Boolean);
				break;
			...
			case "clear_selection_on_clipboard_loss":
				config.mouse.clear_selection_on_clipboard_loss =
					value === "yes" || value === "true";
				break;
		}
	}
	...
	private parsePerformanceConfig(
		...
		switch (key) {
			...
			case "sync_to_monitor":
				config.performance.sync_to_monitor =
					value === "yes" || value === "true";
				break;
		}
	}

	private parseBellConfig(
		config: KittyConfigAST,
		key: string,
		value: string,
	): void {
	...
	private parseWindowLayoutConfig(
		config: KittyConfigAST,
		key: string,
		value: string,
	): void {
		switch (key) {
			case "remember_window_size":
				config.window_layout.remember_window_size =
					value === "yes" || value === "true";
				break;
			case "remember_window_position":
				config.window_layout.remember_window_position =
					value === "yes" || value === "true";
				break;
			...
			case "enabled_layouts":
				config.window_layout.enabled_layouts = value
					.split(",")
					.map((v) => v.trim()) as KittyLayout[];
				break;
			case "window_border_width":
				config.window_layout.window_border_width = value;
				break;
			case "draw_minimal_borders":
				config.window_layout.draw_minimal_borders =
					value === "yes" || value === "true";
				break;
			case "draw_window_borders_for_single_window":
				config.window_layout.draw_window_borders_for_single_window =
					value === "yes" || value === "true";
				break;
			...
			case "single_window_margin_width":
				config.window_layout.single_window_margin_width =
					Number.parseFloat(value);
				break;
			...
			case "single_window_padding_width":
				config.window_layout.single_window_padding_width =
					Number.parseFloat(value);
				break;
			case "placement_strategy":
				config.window_layout.placement_strategy =
					value as KittyPlacementStrategy;
				break;
			...
			case "hide_window_decorations":
				if (value === "yes" || value === "true") {
					...
					config.window_layout.hide_window_decorations = value as
						| "titlebar-only"
						| "titlebar-and-corners";
				}
				break;
			case "window_resize_step_cells":
				config.window_layout.window_resize_step_cells = Number.parseInt(
					value,
					10,
				);
				break;
			case "window_resize_step_lines":
				config.window_layout.window_resize_step_lines = Number.parseInt(
					value,
					10,
				);
				break;
			case "confirm_os_window_close": {
				...
				config.window_layout.confirm_os_window_close = Number.isNaN(numValue)
					? -1
					: numValue;
			...
			case "window_logo_position":
				config.window_layout.window_logo_position =
					value as KittyPlacementStrategy;
				break;
			...
			case "resize_in_steps":
				config.window_layout.resize_in_steps =
					value === "yes" || value === "true";
				break;
			...
			case "window_title_bar":
				if (value === "top" || value === "bottom")
					config.window_layout.window_title_bar = value;
				break;
			...
			case "window_title_bar_align":
				if (value === "left" || value === "center" || value === "right")
					config.window_layout.window_title_bar_align = value;
				break;
			case "window_title_bar_min_windows":
				config.window_layout.window_title_bar_min_windows = Number.parseInt(
					value,
					10,
				);
				break;
	...
	private parseTabBarConfig(
		...
		switch (key) {
			...
			case "tab_bar_show_new_tab_button":
				config.tab_bar.tab_bar_show_new_tab_button =
					value === "yes" || value === "true";
				break;
			case "progress_bar": {
	...
	private parseColorConfig(
		...
		if (key === "background_opacity") {
			...
			config.colors.background_image_linear =
				value === "yes" || value === "true";
			...
			config.colors.background_image_layout =
				value as KittyBackgroundImageLayout;
		} else if (key === "transparent_background_colors") {
			if (!config.colors.transparent_background_colors)
				config.colors.transparent_background_colors = [];
			config.colors.transparent_background_colors.push(value);
		} else if (key in config.colors) {
			(config.colors as unknown as Record<string, string | number | boolean>)[
				key
			] = value;
		}
	}

	private parseOSSpecificConfig(
		config: KittyConfigAST,
		key: string,
		value: string,
	): void {
		switch (key) {
			...
			case "wayland_enable_ime":
				config.os_specific.wayland_enable_ime =
					value === "yes" || value === "true";
				break;
			...
			case "macos_hide_from_tasks":
				config.os_specific.macos_hide_from_tasks =
					value === "yes" || value === "true";
				break;
			case "macos_quit_when_last_window_closed":
				config.os_specific.macos_quit_when_last_window_closed =
					value === "yes" || value === "true";
				break;
			case "macos_window_resizable":
				config.os_specific.macos_window_resizable =
					value === "yes" || value === "true";
				break;
			...
			case "macos_custom_beam_cursor":
				config.os_specific.macos_custom_beam_cursor =
					value === "yes" || value === "true";
				break;
			...
			case "macos_dock_badge_on_bell":
				config.os_specific.macos_dock_badge_on_bell =
					value === "yes" || value === "true";
				break;
			case "linux_display_server":
				config.os_specific.linux_display_server =
					value as KittyLinuxDisplayServer;
				break;
		}
	}

	private parseAdvancedConfig(
		config: KittyConfigAST,
		key: string,
		value: string,
	): void {
		switch (key) {
			...
			case "close_on_child_death":
				config.advanced.close_on_child_death =
					value === "yes" || value === "true";
				break;
			...
			case "remote_control_password":
				if (!config.advanced.remote_control_password)
					config.advanced.remote_control_password = [];
			...
			case "exe_search_path":
				if (!config.advanced.exe_search_path)
					config.advanced.exe_search_path = [];
			...
			case "clone_source_strategies":
				config.advanced.clone_source_strategies = value
					.split(/[,\s]+/)
					.filter(Boolean);
				break;
			case "term":
			...
			case "menu_map":
				...
				break;
			case "action_alias":
				if (!config.advanced.action_alias) config.advanced.action_alias = [];
				config.advanced.action_alias.push(value);
				break;
			case "map_timeout":
				config.advanced.map_timeout = Number.parseFloat(value);
				break;
			case "filter_notification":
				config.advanced.filter_notification = value;
				break;
			case "file_transfer_confirmation_bypass":
